...
%DIF DEL ../../../streamgap-aa/submission/stream_gaps_draft.tex   Mon Oct 12 20:55:11 2015
%DIF ADD stream_gaps_draft.tex                                    Fri Dec 11 18:47:47 2015
\usepackage{times}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsbsy}
...
\def\aap{A \& A}
%DIF 36a36-37
\def\physrep{Physics Reports} %DIF > 
\def\araa{Annual Review of Astron and Astrophys} %DIF > 
%DIF -------
\def\kpc{\,{\rm kpc}}
\def\mas{\,{\rm mas}}
\def\pc{\,{\rm pc}}
...
In recent years there has been much progress made in understanding the structure of stream gaps through the analysis of numerical simulations \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{SiegalGaskins2008,Carlberg2009,YoonJohnstonHogg,Carlberg2012} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Johnston2002,Ibata2002,SiegalGaskins2008,Carlberg2009,YoonJohnstonHogg,Carlberg2012} }%DIFAUXCMD
...
\DIFaddend \cite{ErkalBelokurov2015} gave expressions for the velocity kicks due to a Plummer subhalo on a straight stream segment moving at a fixed relative velocity to the subhalo. If we consider the example of GD-1 and take the model of \cite{Koposov2010} we find that the observed segment of the stream has a radius of curvature of $\sim24\kpc$. \DIFdelbegin \DIFdel{We will see }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015} }%DIFAUXCMD
show }\DIFaddend that for a $10^8M_\odot$ Plummer subhalo with scale radius $r_s=625\pc$ the kicks are important on scales of $\sim10r_s\approx6\kpc$ such that the radius of curvature is comparable to the region over which the kick is important. Similarly, \DIFaddbegin \DIFadd{from \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015} }%DIFAUXCMD
we know that }\DIFaddend the typical distance a stream particle moves during the interaction is $\sim10|\bs{v}|r_s/|\bs{w}-\bs{v}|\approx10r_s/\sqrt{2}\approx4\kpc$ (where $\bs{w}$ is the velocity of the subhalo) such that the curvature of the \DIFdelbegin \DIFdel{motion }\DIFdelend \DIFaddbegin \DIFadd{orbit }\DIFaddend of an individual particle is also important over the interaction. Here we will give more general formulae for the velocity kicks that account for the curved extent of the stream.

\subsection{Plummer subhalo kicks on curved stream}
\DIFaddbegin 

\begin{figure}
$$\DIFaddFL{\includegraphics[width=\columnwidth]{{{plots/fig1_geometry}}}}$$
\caption{\DIFaddFL{Stream-subhalo interaction geometry at the point of closest approach: the right panel is a zoom-in of the region inside the grey dashed square in the left panel. The stream track is a circular orbit of radius $10\kpc$ and circular velocity $220\kms$ shown by the grey line. The red vector $\bs{w}$ shows the projection of the sub-halo velocity. The black position vectors $\bs{x}_0$ and $\bs{x}_i$ show the positions of the stream closest to the subhalo and an arbitrary stream point respectively. The black velocity vectors $\bs{v}_0$ and $\bs{v}_i$ give the velocities of these two stream points. The blue vectors $\bs{b}_0$ and $\bs{b}_i$ are the closest-approach displacement vector and the displacement vector between the subhalo and the arbitrary stream point respectively.
}}
\label{geometry_lb}
\end{figure}


\DIFaddend We expand on the formalism of \cite{ErkalBelokurov2015} by explicitly considering the spatial distribution of the stream. We retain the simplification that each particle moves at a fixed relative velocity to the subhalo during the fly-by. We define the phase-space coordinates of the stream particles as $(\bs{x}_i,\bs{v}_i)$ and the stream point of closest approach as $(\bs{x}_0,\bs{v}_0)$. The \DIFaddbegin \DIFadd{geometry of the interaction is shown in Fig.~\ref{geometry_lb}. The }\DIFaddend vector of closest approach is given by
\begin{equation}
...
\DIFaddend For general subhalo acceleration fields this integral must be calculated numerically via a coordinate transformation to make the limits finite. However, for a Plummer subhalo the velocity kicks may be computed analytically. The potential of a Plummer sphere is given by
\begin{equation}
...
\label{potential}
\end{equation}
with $V_c = 220\kms$ and $q = 0.9$ \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Koposov2010}}%DIFAUXCMD
}\DIFaddend , and the stream here is modelled simply as a circular orbit at $10\kpc$ with a circular velocity of $220\kms$ impacted by a Plummer sphere with velocity $\bs{w}=(0,132,176)\kms$\DIFdelbegin \DIFdel{and impact parameter $b=625\pc$}\DIFdelend \DIFaddbegin \DIFadd{. The choice of velocity is motivated by the work of \mbox{%DIFAUXCMD
\cite{PifflBinney} }%DIFAUXCMD
who constrained the smooth dark matter distribution to have a velocity dispersion at the Sun of $\sigma_i\approx150\kms$ and we assume that the dark matter subhalo distribution function is similar to the smooth dark matter distribution. The impact parameter is $b=-625\pc$ where the minus sign denotes that the subhalo passes inside the circular orbit of the progenitor}\DIFaddend . We show two kicks: one due to a subhalo of mass $10^8M_\odot$ and scale radius $r_s=625\pc$ and one due to a subhalo of mass $10^7M_\odot$ and scale radius $r_s=250\pc$ \DIFaddbegin \DIFadd{(chosen to match the mass-radius relation fitted to the Via Lactea II catalogues of \mbox{%DIFAUXCMD
\cite{Diemand2008}}%DIFAUXCMD
)}\DIFaddend . The distance along the stream in the straight-line approximation is calculated as $\Delta\phi\times10\kpc$.

The differences between the different methods are small. One very noticeable difference is the \DIFaddbegin \DIFadd{amplitude of the }\DIFaddend $\delta v^g_x$ kicks computed using the orbital path of the stream particles, but with the subhalo moving in a straight line, is significantly \DIFdelbegin \DIFdel{larger }\DIFdelend \DIFaddbegin \DIFadd{smaller }\DIFaddend than for the other three cases. This is because the stream particles curve towards the subhalo track and so experience an increased perturbation. This picture is confirmed by moving the subhalo fly-by to outside the stream track which produces an under-estimate for $|\delta v^g_x|$. Additionally, the $\delta v^g_z$ kicks for the full stream and subhalo integration are smaller at large distances from the closest approach. We also show $\bs{v}\cdot\delta\bs{v}^g$ which is very similar for the four cases. \DIFdelbegin \DIFdel{As we will see later }\DIFdelend \DIFaddbegin \DIFadd{This quantity is the change in energy $\delta H$ and is related by a constant scaling to $\Delta E/\Delta E_\mathrm{char}$ plotted in Figure 4 of \mbox{%DIFAUXCMD
\cite{YoonJohnstonHogg}}%DIFAUXCMD
. For many potentials the orbital frequencies are strongly dependent on the energy and only weakly dependent on the other integrals of motion \mbox{%DIFAUXCMD
\citep{BinneyTremaine}}%DIFAUXCMD
. Therefore }\DIFaddend it is this quantity which has the largest effect on the future structure of the stream.  \DIFdelbegin \DIFdel{For }\DIFdelend \DIFaddbegin \DIFadd{Additionally, \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015} }%DIFAUXCMD
argued that for the near-circular orbit case the azimuthal velocity kicks and hence $\bs{v}\cdot\delta\bs{v}^g\approx v_\phi\delta v_\phi^g$ produced the most significant effects on the gap evolution.
}

\DIFadd{In conclusion we find that all the difference between the velocity kicks using different methods is very small, particularly for the $10^7M_\odot$ impact, and probably much smaller than the precision with which line-of-sight velocities will be measured for a number of years. Therefore, we conclude that the straight-line approximation is sufficient. However, the curved approximation has the same computational cost, whilst capturing an additional physical effect, so for }\DIFaddend our calculations in Section~\ref{Sect::Simulation} we will use equation~\eqref{curved} to calculate all kicks. \DIFaddbegin \DIFadd{We note however that we have not explored different impact and stream geometries but rather inspected what we see as a representative case so there may be special cases where the more computationally expensive methods are required.
}\DIFaddend 


\begin{figure*}
$$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{{{plots/fig1_curved_straight_comparison}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{{{plots/fig2_curved_straight_comparison}}}\DIFaddendFL $$
\caption{
A comparison of the velocity kicks calculated with varying levels of complexity \DIFdelbeginFL \DIFdelFL{. }\textbf{\DIFdelFL{Panel (a)}}%DIFAUXCMD
\DIFdelFL{: The stream track is a circular orbit of radius $10\kpc$ and circular velocity $220\kms$ and is shown in black in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{left panel. Two lines are shown in the second through to fifth panel corresponding to two subhaloes, which are each }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{interaction between }\DIFaddendFL a Plummer sphere \DIFdelbeginFL \DIFdelFL{moving at $\bs{w}=(0,132,176)\kms$ with an impact parameter $b=625\pc$. The red arrow in the left panel shows the projection of the velocity vector into the plane. The higher amplitude kicks correspond to the sphere of mass $10^8M_\odot$ }\DIFdelendFL and \DIFdelbeginFL \DIFdelFL{scale radius $r_s=625\pc$ and the lower amplitude kicks correspond to the sphere of mass $10^7M_\odot$ and scale radius $r_s=250\pc$. }\textbf{\DIFdelFL{Panels (b), (c) \& (d)}}%DIFAUXCMD
\DIFdelFL{: The second, third and fourth plots show the velocity kicks as }\DIFdelendFL a \DIFdelbeginFL \DIFdelFL{function of the azimuthal angle from the centre of the galaxy}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{circular stream track as described in Section~\ref{diff_methods}}\DIFaddendFL . The \DIFdelbeginFL \DIFdelFL{black lines }\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{black lines}} \DIFaddendFL show the kicks calculated using equation~\protect\eqref{curved} (i.e. fixed relative velocity during fly-by), whilst the \DIFdelbeginFL \DIFdelFL{red short-dashed lines }\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{red short-dashed lines}} \DIFaddendFL show the kicks computed assuming the stream is a straight line segment along the $y$-axis, the \DIFdelbeginFL \DIFdelFL{blue medium-dashed line }\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{blue medium-dashed lines}} \DIFaddendFL shows the kicks computed using the full path of each particle during the fly-by and the \DIFdelbeginFL \DIFdelFL{green long-dashed lines }\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{green long-dashed lines}} \DIFaddendFL show the kicks computed by integrating each particle in the combined galactic and orbiting Plummer potential. \DIFdelbeginFL \textbf{\DIFdelFL{Panel (e)}}%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Each panel shows two lines corresponding to two different subhalo mass impacts of $10^8M_\odot$ and $10^7M_\odot$. }\textbf{\DIFaddFL{Panels (a), (b) \& (c)}}\DIFaddendFL : The \DIFaddbeginFL \DIFaddFL{first, second and third plots show the Cartesian velocity kicks as a function of the azimuthal angle from the centre of the galaxy. }\textbf{\DIFaddFL{Panel (d)}}\DIFaddFL{: The }\DIFaddendFL rightmost panel shows $\bs{v}\cdot\delta\bs{v}^g$ with the small inset showing the \DIFaddbeginFL \DIFaddFL{absolute }\DIFaddendFL difference between the lines. The black line shows the difference between the full computation and the kicks computed with equation~\protect\eqref{curved}, the red short-dashed line shows the difference between the full computation and the kicks computed assuming the stream is a straight line, and the blue long-dashed shows the difference between the full computation and that including the acceleration of each particle during the fly-by. The dotted blue lines show the angles at which the projection of the velocity vector of the subhalo intersects the stream.
}
...
\begin{figure*}
$$\DIFaddFL{\includegraphics[width=\textwidth]{{{plots/fig3_nfw_plummer_comparison}}}}$$
\caption{
\DIFaddFL{A comparison of the velocity kicks from a Plummer (black solid), a Hernquist (green long-dashed), a NFW (red medium-dashed) and a truncated NFW subhalo (blue short-dashed). }\textbf{\DIFaddFL{Panel (a)}}\DIFaddFL{: The left panel shows the density (top) and mass (bottom) profiles of the four haloes. The three finite mass haloes have the same total mass and the NFW profile has the same mass parameter. The Hernquist, NFW and truncated NFW scale radii are chosen such that at the Plummer scale radius ($r_s$) the same mass is enclosed by the profiles. The truncated NFW halo has a truncation radius equal to twice its scale radius. }\textbf{\DIFaddFL{Panels (b), (c) \& (d)}}\DIFaddFL{: The second, third and fourth panels show the velocity kicks as a function of the distance along the stream.  }\textbf{\DIFaddFL{Panel (e)}}\DIFaddFL{: The rightmost panel shows $\bs{v}\cdot\delta\bs{v}^g$. The stream is a particle train along the $y$-axis with velocity $v_y$. The subhaloes have $4GM=r_s v_y^2$, $r_{s,\mathrm{Hernquist}}=1.36r_s$, $r_{s,\mathrm{NFW}}=1.21r_s$, $r_{s,\mathrm{NFW trunc}}=1.76r_s$, impact parameter $b=\sqrt{3}/2r_s$ and velocity $\bs{w}=(0.3,0.6,0)v_y$. In the second through fifth panel we show in long-dashed purple the kicks from a point-mass (or Kepler potential) with mass equal to the enclosed mass of the Plummer halo at the scale radius.
}}
\label{plummer_nfw}
\end{figure*}


\DIFaddend We now compare the kicks produced by a Plummer profile with those due to other astrophysically interesting profiles for which we compute the kicks numerically. We consider kicks due to Hernquist profiles \citep{Hernquist} and truncated Navarro-Frenk-White \citep[NFW,][]{NFW1996} profiles. The density profile for the truncated NFW halo is given by
\begin{equation}
\rho_{\rm NFW}(r) = \frac{M}{4\pi r_s^3}\Big(\frac{r}{r_s}\Big)^{-1}\Big(1+\frac{r}{r_s}\Big)^{-2}\mathrm{sech}\Big(\frac{r}{r_t}\Big).
\end{equation}
$r_t$ is the truncation radius. For non-zero $r_t$ the forces and potential must be computed numerically and clearly for $r_t\rightarrow\infty$ the form reduces to the well-known NFW profile. \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Hayashi2003} }%DIFAUXCMD
find that tidally-stripped dark-matter haloes have a polynomial truncation of $\rho\propto r^{-6}$ at large radii so the exponential truncation should be treated as the most extreme truncation and more realistic NFW halos will lie somewhere between our truncated and non-truncated lines.
}\DIFaddend 
...
%DIFDELCMD < %%%
\begin{displaymath}\DIFdelFL{\includegraphics[width=\textwidth]{{{plots/fig2_nfw_plummer_comparison}}}}\end{displaymath}
%DIFAUXCMD
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{A comparison of the velocity kicks from a Plummer (black solid), a Hernquist (green long-dashed), a NFW (red medium-dashed) and a truncated NFW subhalo (blue short-dashed). }\textbf{\DIFdelFL{Panel (a)}}%DIFAUXCMD
...
\DIFdelFL{: The second, third and fourth panels show the velocity kicks as a function of the distance along the stream.  }\textbf{\DIFdelFL{Panel (e)}}%DIFAUXCMD
\DIFdelFL{: The rightmost panel shows $\bs{v}\cdot\delta\bs{v}^g$. The stream is a particle train along the $y$-axis with velocity $v_y$. The subhaloes have $4GM=r_s v_y^2$, $r_{s,\mathrm{Hernquist}}=1.36r_s$, $r_{s,\mathrm{NFW}}=1.21r_s$, $r_{s,\mathrm{NFW trunc}}=1.76r_s$, impact parameter $b=\sqrt{3}/2r_s$ and velocity $\bs{w}=(0.3,0.6,0)v_y$. In the second through fifth panel we show in long-dashed purple the kicks from a point-mass (or Kepler potential) with mass equal to the enclosed mass of the Plummer halo at the scale radius.
}}
%DIFAUXCMD
%DIFDELCMD < \label{plummer_nfw}
...
The stream stretches along the direction $\bs{n}$ and in each tail the frequency distribution is very narrow such that, at a fixed time, the locations of the particles in the stream are well approximated by a single angle coordinate $\theta_{||}=\btheta\cdot\bs{n}$. We assume that the kicks are functions of this single variable. \DIFaddbegin \DIFadd{Similarly, the frequency distribution can be described by the single frequency coordinate $\Omega_{||}=\boldsymbol{\Omega}\cdot\bs{n}$. }\DIFaddend This assumption is equivalent to assuming the stream is very cold as we are ignoring any spatial extent of the stream perpendicular to the streaming direction. \DIFaddbegin \DIFadd{The distribution perpendicular to these narrow distributions are described using the angle and frequency coordinates
}\begin{equation}
\DIFadd{\begin{split}
...
\DIFaddend If the kick occurred a time $t_g$ ago the angles and frequencies of a stream particle are given by
...
$$\DIFaddFL{\includegraphics[width=\columnwidth]{{{plots/fig4_tilted_gap_morphology}}}}$$
\caption{
\DIFaddFL{Real-space distribution of stream and gap zoom-in for the $10^8M_\odot$ subhalo fly-by $880\Myr$ after impact.
}}
\label{GapDiagram}
\end{figure}

\begin{table}
\caption{\DIFaddFL{Parameters used for the $N$-body simulation described in Section~\ref{Sect::Simulation}. The parameters above the divide refer to the stream progenitor system whilst those below refer to the dark-matter subhalo. The progenitor of the stream follows a King profile, whilst the subhalo follows a Plummer profile.}}
\begin{tabular}{lll}
\hline
\DIFaddFL{Stream
}&\DIFaddFL{Mass}&\DIFaddFL{$10^5 M_\odot$}\\
&\DIFaddFL{Core radius}&\DIFaddFL{$13\pc$}\\
&\DIFaddFL{$W_0$}&\DIFaddFL{5}\\
&\DIFaddFL{Particle number}&\DIFaddFL{$10^6$}\\
&\DIFaddFL{Smoothing length}&\DIFaddFL{$1\pc$}\\
&\DIFaddFL{Initial position}&\DIFaddFL{$(30,0,0)\kpc$ }\\
&\DIFaddFL{Initial velocity}&\DIFaddFL{$(0, 105.75, 105.75)\kms$ }\\
\hline
\DIFaddFL{Subhalo}&\DIFaddFL{Mass}&\DIFaddFL{$10^7 M_\odot,10^8 M_\odot$}\\
&\DIFaddFL{Scale radius}&\DIFaddFL{$250 \pc,625 \pc$}\\
&\DIFaddFL{Impact parameter}&\DIFaddFL{$0$}\\
&\DIFaddFL{Velocity at impact}&\DIFaddFL{$200\kms\,\perp$ to stream velocity }\\
&\DIFaddFL{Impact time}&\DIFaddFL{$10\Gyr$ after stream progenitor released}\\
&\DIFaddFL{Insertion time}&\DIFaddFL{$100\Myr$ before impact}\\
&\DIFaddFL{Removal time}&\DIFaddFL{$100\Myr$ after impact}\\
\hline
\end{tabular}
\label{SimParamsTable}
\end{table}


\begin{figure*}
$$\DIFaddFL{\includegraphics[width=\textwidth]{{{plots/fig5_tilted_diff_map}}}}$$
\caption{
\DIFaddFL{Difference histograms between the unperturbed and perturbed streams at $t=0.88\Gyr$ after impact: the }\textbf{\DIFaddFL{top row}} \DIFaddFL{shows the distribution in angles and }\textbf{\DIFaddFL{bottom row}} \DIFaddFL{the frequencies. The right panels show the distributions in the parallel-perpendicular space where `parallel' is the distance along the stream direction and `perpendicular' is the Euclidean distance from the stream direction vector. Zero-points correspond to the gap centre as described in Sec.~}\protect\DIFaddFL{\ref{Sec::ActAngFreq}. The bin size for the angles is $0.03\rad$ ($0.0012\rad$ for the perpendicular direction) and $0.015\Gyr^{-1}$ for the frequencies ($7.5\times10^{-4}\Gyr^{-1}$ for the perpendicular direction).
}}
\label{tilted_diff_map}
\end{figure*}
\DIFaddend To investigate the formation of stream gaps in actions, angles and frequencies we ran two $N$-body simulations. The simulations were run using \textsc{Gadget-3} which is an improved version of \textsc{Gadget-2} \citep{Gadget2}. \DIFaddbegin \DIFadd{The parameters of the simulation are summarised in Table~\ref{SimParamsTable}. }\DIFaddend We first generated a stream by disrupting a King profile with $W_0=5$, a mass of $10^5 M_\odot$ and a core radius of $r_c=13\pc$. The cluster was modelled with $10^6$ particles and a smoothing length of $1\pc$.
The galactic potential was chosen to be a logarithmic halo of the form in equation~\eqref{potential} with $V_c = 220\kms$ and $q = 0.9$. The cluster was placed on an eccentric orbit with a pericentre of $15\kpc$ and an apocentre of $30\kpc$. It was released around apocentre at a position of $(30,0,0)\kpc$ with an initial velocity of $(0, 105.75, 105.75)\kms$ \DIFaddbegin \DIFadd{such that the orbital period of the progenitor was $\sim650\Myr$}\DIFaddend . The simulation was run for $10\Gyr$ after which a long cold stream of total length $\sim 300^\circ$ was generated. This stream was then used in two simulations where it was impacted by two different Plummer subhaloes: a large subhalo with a mass of $10^8 M_\odot$ and a scale radius of $r_s = 625 \pc$, and a small subhalo with a mass of $10^7 M_\odot$ and $r_s = 250 \pc$. The impact point was in the trailing arm of the stream when it was near pericentre with an impact parameter of $b=0$ and a velocity which was perpendicular to the stream plane at the point of impact with a magnitude of $200\kms$. The subhalo was inserted into the simulation $100\Myr$ before impact and removed $100\Myr$ after impact to remove the possibility of multiple interactions with the stream. \DIFaddbegin \DIFadd{The stream-subhalo interaction time is of the order of $r_s/|\bs{w}-\bs{v}|$ where $\bs{v}$ is the velocity of the gap centre at impact. For the considered interaction, this expression gives $\sim 1.5\Myr$ which is significantly smaller than the time the subhalo is in the simulation. }\DIFaddend Each simulation was then evolved for a further $5\Gyr$.  In Figure~\ref{GapDiagram} we show the full stream in the $(x,y)$ plane and a zoom-in of the gap centre at $880\Myr$ after impact for the large subhalo. It is this simulation that we inspect throughout the paper and we have used the smaller subhalo simulation for validation.



This section is split into several subsections. In Section~\ref{Sec::ActAngFreq} we project the $N$-body simulation of the perturbed stream into action, angle and frequency coordinates. We then compute the angle and frequency kicks from Section~\ref{Sec::Formalism_angfreq} and compare to the \DIFdelbegin \DIFdel{simulation }\DIFdelend \DIFaddbegin \DIFadd{perturbed and unperturbed simulations }\DIFaddend in Section~\ref{Sec::AngFreqKicks}. In Section~\ref{Sec::PerturbModel} we perturb the unperturbed snapshot using the numerically computed kicks and compare with the simulation. We develop an analytic expression for the frequency kicks in Section~\ref{Sect::analytic}. In Section~\ref{Sect::StreamGrowth} we discuss the stages of stream growth in angle space. In Section~\ref{Sect::anggapamp} we show how the angle kicks are related to the gap size in angle space and finally in Section~\ref{Sect::GapSize} show how the angle gap size is related to the spatial gap size.


\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < %%%
\begin{displaymath}\DIFdelFL{\includegraphics[width=\columnwidth]{{{plots/fig3_tilted_gap_morphology}}}}\end{displaymath}
%DIFAUXCMD
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Real-space distribution of stream and gap zoom-in for the $10^8M_\odot$ subhalo fly-by.
}}
%DIFAUXCMD
%DIFDELCMD < \label{GapDiagram}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{\DIFdelbegin \DIFdel{Action, angle }\DIFdelend \DIFaddbegin \DIFadd{Angle }\DIFaddend \& frequency structure}\label{Sec::ActAngFreq}
Here we investigate the angle \DIFdelbegin \DIFdel{, action }\DIFdelend and frequency structure of the stream. We take the snapshot of the stream at $880\Myr$ after impact and compute the angles \DIFdelbegin \DIFdel{, frequencies and actions }\DIFdelend \DIFaddbegin \DIFadd{and frequencies }\DIFaddend for particles in the trailing stream tail (defined by a $\sim180^\circ$-long segment in azimuthal angle). We also perform the same calculation for the stream evolved without a subhalo flyby. In Fig.~\ref{tilted_diff_map} we show histograms of the difference in the density in angle \DIFdelbegin \DIFdel{, frequency and action space }\DIFdelend \DIFaddbegin \DIFadd{and frequency }\DIFaddend between the perturbed and unperturbed simulation. We plot all quantities with respect to the coordinates of the gap centre which is defined by taking the unperturbed stream at the time of impact, computing the median velocity of the stream particles within \DIFdelbegin \DIFdel{10 pc }\DIFdelend \DIFaddbegin \DIFadd{$10\pc$ }\DIFaddend of the impact point, and then integrating the orbit starting at the impact point with this velocity. In the angle and frequency distributions there is a clear under-density along the stream direction\DIFdelbegin \DIFdel{. In action space we also observe an under-density at the centre of the distributions but the exact geometry of the under and over-densities is complex and clearly depends on the nature of the stream track and subhalo properties. We will discuss this further in Section~\ref{Sect::analytic} and }\DIFdelend \DIFaddbegin \DIFadd{, and we observe that the spreads in $|\boldsymbol{\theta}_\perp|$ and $|\boldsymbol{\Omega}_\perp|$ are significantly smaller than $\theta_{||}$ and $\Omega_{||}$ justifying our modelling assumption. In }\DIFaddend Appendix~\ref{Appendix::Analytic} \DIFaddbegin \DIFadd{we plot similar distributions in action space which exhibit very similar features}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD < %%%
\begin{displaymath}\DIFdelFL{\includegraphics[width=\textwidth]{{{plots/fig4_tilted_diff_map}}}}\end{displaymath}
%DIFAUXCMD
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Difference histograms between the unperturbed and perturbed streams at $t=0.88\Gyr$: the }\textbf{\DIFdelFL{top row}} %DIFAUXCMD
\DIFdelFL{shows the distribution in actions, }\textbf{\DIFdelFL{middle row}} %DIFAUXCMD
\DIFdelFL{the angles and }\textbf{\DIFdelFL{bottom row}} %DIFAUXCMD
\DIFdelFL{the frequencies. The right panels in the second and third rows show the distributions in the parallel-perpendicular space where `parallel' is the distance along the stream direction and `perpendicular' is the Euclidean distance from the stream direction vector. Zero-points correspond to the gap centre as described in Sec.~}%DIFDELCMD < \protect%%%
\DIFdelFL{\ref{Sec::ActAngFreq}.
}}
%DIFAUXCMD
%DIFDELCMD < \label{tilted_diff_map}
%DIFDELCMD < \end{figure*}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In Figure~\ref{angfreq_dist} we plot the angles and frequencies for the unperturbed and perturbed trailing stream distributions. The unperturbed simulation consists of a spur \DIFaddbegin \DIFadd{(marked in blue in Figure~\ref{angfreq_dist}) }\DIFaddend due to each stripping event. The material stripped in each event forms an approximately vertical line in this space before gradually twisting clockwise as the differential frequency effects take hold. The material on the far left was stripped earliest. We see that the subhalo produces the expected S-shape which also twists clockwise in time due to the differential frequency effects. At \DIFaddbegin \DIFadd{the impact time (}\DIFaddend $t=0$\DIFaddbegin \DIFadd{) }\DIFaddend there is a clear gap in frequency but only a relatively modest gap in angle. However, the twisting \DIFaddbegin \DIFadd{naturally }\DIFaddend produces a larger gap in the angles. The twisting also causes some perturbed material to overtake the unperturbed material \DIFaddbegin \DIFadd{(highlighted by the blue box in Figure~\ref{angfreq_dist})}\DIFaddend .

\begin{figure}
$$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{{{plots/fig5_ang_freq_dists}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{{{plots/fig6_ang_freq_dists}}}\DIFaddendFL $$
\caption{
Frequency-angle distributions for unperturbed (black) and perturbed (red) trailing tail at three different times \DIFaddbeginFL \DIFaddFL{after the subhalo impact}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{Each row shows a }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The three rows show the frequency and angle distributions with respect to the centre of the gap at three }\DIFaddendFL different \DIFdelbeginFL \DIFdelFL{time }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{times which are }\DIFaddendFL written in the right column. The three columns from left to right show the radial angle against radial frequency, the azimuthal angle against azimuthal frequency and vertical angle against vertical frequency.
%DIF >  The bottom row shows the normalized 1D angle distributions at the three times with the dotted line corresponding to the $0\Gyr$ case, the dashed corresponding to $0.88\Gyr$ and the solid line corresponding to $5\Gyr$.
\DIFaddbeginFL \DIFaddFL{In the top right panel we show a blue line that lies along one of the `spurs' in the unperturbed model. In the bottom middle panel we highlight those kicked particles that are now overtaking other stream particles.
}\DIFaddendFL }
\label{angfreq_dist}
\end{figure}
...
We begin by taking the snapshot of the unperturbed stream at the impact time and form a stream track \DIFaddbegin \DIFadd{in angle-frequency space }\DIFaddend by fitting a spline to the stream particles. We then compute the vector $\bs{n}$ from this track and at each point along the stream track compute $\theta_{||}=\btheta\cdot\bs{n}$ as well as $\delta\bs{\Omega}^g(\theta_{||})$ and $\delta\bs{\theta}^g(\theta_{||})$ from equation~\eqref{eq:kickfreqangle} by finite differencing in velocity at fixed position. We show the resultant kicks in Fig.~\ref{kicks_nzero}. We also show the velocity, angle and frequency kicks computed from the simulation. The perturbed velocities at impact are found by integrating the simulation \DIFdelbegin \DIFdel{at }\DIFdelend \DIFaddbegin \DIFadd{backwards from }\DIFaddend $880\Myr$ after impact\DIFdelbegin \DIFdel{backwards}\DIFdelend . In the case of the frequency kicks we simply difference the perturbed and unperturbed simulation. In the case of the angles we calculate the angles and frequencies of the perturbed simulation snapshot at \DIFdelbegin \DIFdel{$t=880\Myr$ }\DIFdelend \DIFaddbegin \DIFadd{$880\Myr$ after impact }\DIFaddend and rewind to the impact point before differencing with the unperturbed snapshot.
\DIFaddbegin 

\DIFaddend The fact the velocity kicks match well demonstrates that the assumptions made to derive the formulae in Section~\ref{Sect::Formalism} are valid whilst the use of the impulse approximation and the linear approximation from equation~\eqref{eq:kickfreqangle} is validated by the match of the frequency kicks. Interestingly the amplitude of the velocity kicks found from the numerical calculation overpredicts that in the simulation far from the impact centre. This corresponds to the difference observed in Fig.~\ref{curved_straight} between computing the kicks using the full stream track and subhalo orbit (`full', green dashed line) and assuming the relative velocity between each stream particle and subhalo is fixed during the fly-by (`curved', black).

The angle kicks are perhaps a less satisfying match than the velocity and frequency kicks. This may be due to the assumption of the impulse approximation or the neglected stream dispersion. We performed several checks of the accuracy of the angle computation: we measured the fluctuations in the \DIFdelbegin \DIFdel{angle calculation for a single orbitabout }\DIFdelend \DIFaddbegin \DIFadd{angles about a straight line using a series of samples from a $300\Gyr$ segment of the progenitor's orbit. If computed exactly, the angles should lie along }\DIFaddend a straight line \DIFdelbegin \DIFdel{in angle space and }\DIFdelend \DIFaddbegin \DIFadd{so deviations about this are due to numerical error. We }\DIFaddend also compared a snapshot evolved in angle-frequency space \DIFaddbegin \DIFadd{(i.e. using equation~}\eqref{angleactdef}\DIFadd{) }\DIFaddend with a later simulation snapshot \DIFaddbegin \DIFadd{projected into angle-frequency space}\DIFaddend . These tests show that the angles are computed to an accuracy of \DIFdelbegin \DIFdel{$\sim10^{-3}\rad$ }\DIFdelend \DIFaddbegin \DIFadd{$\sim5\times10^{-4}\rad$ }\DIFaddend such that the discrepancy observed here cannot be due to errors in the computation.

\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD < %%%
\begin{displaymath}\DIFdelFL{\includegraphics[width=\textwidth]{{{plots/fig6_tilted_angfreq_kicks}}}}\end{displaymath}
%DIFAUXCMD
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Angle and frequency kicks: the points are a random sample of $1000$ particles from the stream and show the kicks found from the simulations whilst the lines show those calculated under the impulse approximation. The functional form for the kicks in both the angles and frequencies are very similar to that in velocities. Also, the angle and frequency kicks computed from the simulation match the numerical results well.
}}
%DIFAUXCMD
%DIFDELCMD < \label{kicks_nzero}
%DIFDELCMD < \end{figure*}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Model-simulation comparison}\label{Sec::PerturbModel}
...
In Fig.~\ref{model_nzero} we show the frequency distributions of the unperturbed simulation, the perturbed simulation and our model constructed by perturbing the frequencies of the unperturbed simulation. The match is very good and there is a clear gap in all the \DIFdelbegin \DIFdel{frequencies.
}\DIFdelend \DIFaddbegin \DIFadd{components of the frequencies. We also show the parallel frequency distribution which exhibits a clear gap and the perpendicular frequency distribution which is slightly broader in the perturbed case than the unperturbed case reflecting the increase in velocity dispersion of the stream due to the subhalo.
}\DIFaddend 

In Fig.~\ref{model_severaltimes} we plot the unperturbed and perturbed stream in angle space at four different times ($0\Gyr,0.88\Gyr,3\Gyr$ and $5\Gyr$ after impact) along with the model constructed by perturbing the unperturbed stream with the impulse approximation using equation~\eqref{eq:kickfreqangle}. Additionally we show the difference between the perturbed and unperturbed angle distributions. We see at all times there is a gap in $\theta_z$ and $\theta_R$ with an overdensity at $0\Gyr$ after impact in $\theta_\phi$. The gap grows in time in all angles, and our model matches the distributions well at all times\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, despite the fact that the angle kicks do not match the simulation particularly well (see Fig.~\ref{kicks_nzero}). This must be because the angle distributions are a combination of the frequency and angle kicks and the frequency kicks dominate at late times.
}\DIFaddend 


\begin{figure*}
$$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{{{plots/fig7_tilted_model_perturbed}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{{{plots/fig7_tilted_angfreq_kicks}}}\DIFaddendFL $$
\caption{
\DIFdelbeginFL \DIFdelFL{Frequency distributions for the perturbed }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Angle }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{unperturbed stream model. The red histograms show }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{frequency kicks: }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{unperturbed stream, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{coloured points are a random sample of $1000$ particles from }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{green show the perturbed }\DIFdelendFL stream and \DIFaddbeginFL \DIFaddFL{show }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{blue }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{kicks found from the simulations whilst the black lines }\DIFaddendFL show \DIFaddbeginFL \DIFaddFL{those calculated under }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{unperturbed stream kicked }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{impulse approximation. The functional form for the kicks }\DIFaddendFL in \DIFaddbeginFL \DIFaddFL{both the angles and }\DIFaddendFL frequencies \DIFdelbeginFL \DIFdelFL{using equations~}%DIFDELCMD < \protect\eqref{eq:kickfreqangle}%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{are very similar to that in velocities}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{Also, the angle and frequency kicks computed from the simulation match the numerical results well.
}\DIFaddendFL }
\DIFaddbeginFL \label{kicks_nzero}
\end{figure*}

\begin{figure*}
$$\DIFaddFL{\includegraphics[width=\textwidth]{{{plots/fig8_tilted_model_perturbed}}}}$$
\caption{
\DIFaddFL{Frequency distributions for the perturbed and unperturbed stream model. The red histograms show the unperturbed stream, the green show the perturbed stream and the blue show the unperturbed stream kicked in frequencies using equations~}\protect\eqref{eq:kickfreqangle}\DIFaddFL{. The left three panels show the distributions in the orbital frequencies with respect to the gap centre. The fourth panel shows the frequency distribution along the stream direction $\hat{\boldsymbol{n}}$ whilst the fifth panel shows the distribution perpendicular to this.
}}
\DIFaddendFL \label{model_nzero}
\end{figure*}

\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{figure}[h]
\DIFaddendFL $$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{{{plots/fig8_ang_later_times}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{{{plots/fig9_ang_later_times}}}\DIFaddendFL $$
\caption{
...
We \DIFaddbegin \DIFadd{have seen that the angle distributions formed from kicking the unperturbed simulation match the perturbed simulation well at late times suggesting that the angle kicks are insignificant and dominated by the frequency kicks. To understand this, we }\DIFaddend now develop some understanding of the \DIFaddbegin \DIFadd{relative }\DIFaddend magnitude of the angle \DIFdelbegin \DIFdel{, frequency and action }\DIFdelend \DIFaddbegin \DIFadd{and frequency }\DIFaddend kicks by inspecting some cases where we can calculate these quantities analytically. \DIFaddbegin \DIFadd{In Appendix~\ref{Appendix::Analytic} we also present some approximate analytic results for the action kicks due to a subhalo fly-by.
}\DIFaddend 
...
\begin{figure}
$$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{{{plots/fig9_analytic_freq}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{{{plots/fig10_analytic_freq}}}\DIFaddendFL $$
\caption{
...
\DIFdelbegin \DIFdel{In Appendix~\ref{Appendix::Analytic} we present some approximate analytic results for the action and angle kicks due to a subhalo fly-by, and argue that
the angle kicks }\DIFdelend \DIFaddbegin \DIFadd{From our investigations there do not appear to be any particularly neat or understandable analytic expressions we can derive for the changes to the angle coordinates. What follows are some loose arguments to get a handle on the amplitude of the angle kicks relative to the frequency kicks. We inspect the harmonic oscillator (which is also appropriate for small oscillations about a circular orbit and perpendicular to the plane). In this case, the angle coordinate $\theta_i$ is given by
}\begin{equation}
\DIFadd{\tan\theta_i = \frac{\Omega_i x_i}{v_i},
}\end{equation}
\DIFadd{where $x_i$ is the position coordinate relative to the minimum, $v_i$ is the velocity coordinate. For the angle kicks we find that
}\begin{equation}
\DIFadd{\delta\theta^g_i\approx \frac{\tan\theta_i}{1+\tan^2\theta_i}\Big(\frac{\delta\Omega^g_i}{\Omega_i}-\frac{\delta v_i}{v_i}\Big),
}\end{equation}
\DIFadd{For a true harmonic oscillator the change in frequency is identically zero such that this expression can be reduced to
}\begin{equation}
\DIFadd{\delta\theta^g_i\approx -\frac{\delta v_i}{\sqrt{E}}\sin\theta_i = -\frac{1}{2}\frac{\delta v_i}{v_i}\sin 2\theta_i,
}\end{equation}
\DIFadd{where $E$ is the energy of the oscillation. We see that near a turning point ($v_i\approx0$) the angle kicks are largest and of order $\delta v_i/\sqrt{E}$ and near the midpoints ($x_i\approx0$) the angle kicks are approximately zero. As expected the magnitude of the angle kicks are phase-dependent. Using equation~}\eqref{dOm_an} \DIFadd{we see that
}\begin{equation}
\DIFadd{\frac{\delta\theta_i^g}{\delta\Omega_i^g}\approx\Omega_i^{-1}\Big(\frac{V_c}{|\bs{v}_i|}\Big)^2\sin 2\theta_i,
}\end{equation}
\DIFadd{where we have neglected geometric factors. The final two terms are of order unity such that these rather loose arguments convince us that this ratio is approximately the period of the oscillation in dimension $i$. From inspection of Fig.~\ref{kicks_nzero} we see that the ratio of the angle to frequency kicks is of order $0.1\Gyr$ which corresponds approximately to $1/\Omega$ as $\Omega_i\approx10\Gyr^{-1}$. From inspection of the simulation it appears $|\delta\theta^g_i|\approx\delta\Omega^g_i/\Omega_i$ within factors of $3$ for $|\Delta\theta_{||}|<1\rad$. Therefore, we have ascertained that the angle kicks }\DIFaddend are dominated by the frequency kicks after approximately one period.

\DIFaddbegin \DIFadd{The study of \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015} }%DIFAUXCMD
showed that gaps only begin to form in the stream after $\sim$ one radial period so an observed gap is always in the regime where the frequency kicks are dominating and when modelling we can essentially neglect the angle kicks.
}

\DIFaddend \subsection{Stages of stream growth}\label{Sect::StreamGrowth}
...
\begin{figure}
$$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{{{plots/fig10_tilted_gap_size}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{{{plots/fig11_tilted_gap_size}}}\DIFaddendFL $$
\caption{
Gap size in parallel angle as a function of time. The \DIFaddbeginFL \DIFaddFL{zero-point in time is the subhalo impact time. The }\DIFaddendFL black line shows the gap measured from the simulation and the dashed red line shows the best fitting power-law line for $t<5\Gyr$. We also mark on the three phases of the gap growth from \protect\cite{ErkalBelokurov2015}.
}
\label{gap_size}
\end{figure}

In Fig.~\ref{gap_size} we plot the size of the gap in parallel angle as a function of time for the simulated stream. This is defined as the difference between the parallel angles of the points where the perturbed and unperturbed densities are equal. We produce a smooth difference histogram using \DIFaddbegin \DIFadd{a Gaussian }\DIFaddend kernel density estimation with a bandwidth of $\Gamma = 0.04\rad\Gyr^{-1}t+0.01\rad$ for $t<1\Gyr$ and $0.05\rad$ otherwise and then fit a spline to the resulting distribution\DIFdelbegin \DIFdel{(we use this smoothing kernelfor all future results on the gap size and minimum /maximum densities in the stream with an additional factor in the bandwidth of $0.5\log_{10}(M/10^6M_\odot)$)}\DIFdelend \DIFaddbegin \DIFadd{. With this choice of kernel, there are $\sim1000$ particles within one kernel standard deviation at the gap minimum and $\sim5000$ at the maxima}\DIFaddend . We also show a best-fit power-law line ($\Delta\theta_{||}\propto t^{0.58}$) for the segment $t<5\Gyr$ as well as the time-scales for the different stages of gap growth. The compression phase lasts for approximately a radial period. The time-scale on which the caustic becomes important is computed from equation (26) of \cite{ErkalBelokurov2015}. In this example the expansion phase only lasts for $\sim200\Myr$ but lowering the subhalo mass to $10^7M_\odot$ increases its length to $\sim600\Myr$. We note that there is a gap in parallel angle at \DIFaddbegin \DIFadd{the impact time (}\DIFaddend $t=0$\DIFaddbegin \DIFadd{)}\DIFaddend . This does not correspond to a spatial gap but is due to the angle kicks from the subhalo that have shifted the stars to slightly different orbital phases.
...
\begin{figure}
$$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{{{plots/fig11_d2dthetadtheta2}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{{{plots/fig12_d2dthetadtheta2}}}\DIFaddendFL $$
\caption{
Derivatives of the parallel angle kicks: the \DIFdelbeginFL \DIFdelFL{solid blue line }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{top panel }\DIFaddendFL shows the \DIFdelbeginFL \DIFdelFL{second derivative of the kicks whilst the dotted red line shows the }\DIFdelendFL expression from equation~\eqref{deriv} \DIFdelbeginFL \DIFdelFL{. The solid }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in dashed }\DIFaddendFL red \DIFdelbeginFL \DIFdelFL{line shows }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{alongside }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{scaled }\DIFdelendFL density contrast \DIFdelbeginFL \DIFdelFL{from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{simulation}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{perturbed density in parallel angle space divided by the unperturbed density minus $1$) in solid red}\DIFaddendFL . The \DIFdelbeginFL \DIFdelFL{black }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{grey }\DIFaddendFL lines show the zero line and the approximate location of the peak density contrast. \DIFaddbeginFL \DIFaddFL{The bottom panel shows the second derivative of the parallel angle kicks with respect to the parallel angle.
}\DIFaddendFL }
\label{secondderiv}
\end{figure}

The second derivative is zero around \DIFdelbegin \DIFdel{$\delta\theta_{||}\approx0.16\rad$ }\DIFdelend \DIFaddbegin \DIFadd{$\delta\theta_{||}\approx0.14\rad$ }\DIFaddend and equation~\eqref{deriv} crosses zero around $\delta\theta_{||}\approx0.1\rad$. These correspond well to the observed initial amplitude of the parallel-angle gap size $(\sim0.2\rad)$. \DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{In Fig.~\ref{secondderiv} we }\DIFaddend also show the density contrast computed from the simulations which correlates well with equation~\eqref{deriv}.
\DIFdelbegin \DIFdel{As the angle kick depends upon $\theta_{||}$ and is larger at small $\Delta\theta_{||}$ and }\DIFdelend \DIFaddbegin 

\DIFadd{We can now understand why the initial gap size in parallel angle is much greater than the magnitude of the kicks in parallel angle. Fig.~\ref{kicks_nzero} shows how the angle kicks depend upon $\Delta\theta_{||}$ and we see that the kicks peak at small $|\Delta\theta_{||}|$ with a long tail at larger $|\Delta\theta_{||}|$. Additionally, }\DIFaddend the range of $\Delta\theta_{||}$ over which the kicks are important is larger than the \DIFdelbegin \DIFdel{angle kicks (see Fig. ~\ref{model_nzero}), }\DIFdelend \DIFaddbegin \DIFadd{magnitude of angle kicks. When considering }\DIFaddend the gap that forms \DIFaddbegin \DIFadd{we have to consider the collective effects of many particles over a large range in $|\Delta\theta_{||}|$ being kicked a small amount and the gap that forms }\DIFaddend is larger than perhaps expected. If the angle kicks were large compared to the range in $\Delta\theta_{||}$ over which the kicks are important the gap size would be approximately the amplitude of the angle kicks. Therefore, we conclude that there is not a one-to-one correspondence between the amplitude of the angle kicks and the initial parallel angle gap size, and the latter can be significantly greater than the former.

\subsection{Physical and parallel angle gap size}\label{Sect::GapSize}

\DIFaddbegin \begin{figure}
$$\DIFaddFL{\includegraphics[width=\columnwidth]{{{plots/fig13_real_vs_angle_properties}}}}$$
\caption{
\DIFaddFL{Gap properties in real and angle space as a function of time. The zero-point of time is the subhalo impact time. In the }\textbf{\DIFaddFL{top panel}} \DIFaddFL{we show the gap size in the azimuthal angle of a plane fitted through the stream in solid black and the gap size in parallel angle in dashed red. In the }\textbf{\DIFaddFL{bottom panel}} \DIFaddFL{we show the minimum and maximum density contrast in these two spaces.
}}
\label{real_v_angle}
\end{figure}
\DIFaddend Our discussion so far has focussed on the size of the gap in parallel angle. Whilst this is an appropriate space for understanding general stream morphology it is awkward to see the relationship with the observed stream properties. We briefly investigate this by projecting the model back into \DIFdelbegin \DIFdel{real-space}\DIFdelend \DIFaddbegin \DIFadd{configuration space}\DIFaddend . For this purpose we use the publicly-available torus code from \DIFdelbegin \DIFdel{Binney \& McMillan (2015, in prep., }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{BinneyMcMillan2015} }%DIFAUXCMD
(}\DIFaddend \href{https://github.com/PaulMcMillan-Astro/Torus}{https://github.com/PaulMcMillan-Astro/Torus}). We \DIFaddbegin \DIFadd{require a different numerical algorithm to map from angle-action space back into configuration space as the algorithm from \mbox{%DIFAUXCMD
\cite{SandersBinney2014} }%DIFAUXCMD
transforms from configuration space to angle-action space.
}

\DIFadd{We }\DIFaddend construct eight tori on the corners of an action-space box that encompasses the full perturbed distributions and interpolate the frequencies to find the actions of the perturbed particles. As described in \DIFdelbegin \DIFdel{Binney \& McMillan (2015, in prep.) }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{BinneyMcMillan2015} }%DIFAUXCMD
}\DIFaddend we interpolate the Fourier coefficients of the tori in action-space to construct a torus of the required actions for each particle and request the $(\bs{x},\bs{v})$ corresponding to the angles of the particle. \DIFaddbegin \DIFadd{Each torus is constructed with a relative tolerance in the actions of $2\times10^{-6}$ and recovers the positions and velocities of the particles to an absolute accuracy of $\sim0.01\kpc$ and $\sim0.1\kms$ respectively. This accuracy is sufficient for inspecting the density and width of the gap which is of order a few $\kpc$ after a $\Gyr$.
}\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < %%%
\begin{displaymath}\DIFdelFL{\includegraphics[width=\columnwidth]{{{plots/fig12_real_vs_angle_properties}}}}\end{displaymath}
%DIFAUXCMD
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Gap properties in real and angle space. In the }\textbf{\DIFdelFL{top panel}} %DIFAUXCMD
\DIFdelFL{we show the gap size in the azimuthal angle of a plane fitted through the stream in solid black and the gap size in parallel angle in dashed red. In the }\textbf{\DIFdelFL{bottom panel}} %DIFAUXCMD
\DIFdelFL{we show the minimum and maximum density contrast in these two spaces.
}}
%DIFAUXCMD
%DIFDELCMD < \label{real_v_angle}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The discrepancy between our simulation and the picture of \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015} }%DIFAUXCMD
seems to be due to our stream having a non-zero velocity dispersion. This means each stripping event produces stream particles with a range of energies and upstream (i.e. closer to the progenitor) particles with large energy differences from the progenitor can pass through the stream and fill in a downstream (i.e. further from the progenitor) gap. Inspection of Fig.~\ref{angfreq_dist} shows that if the parallel frequency kicks are greater than the spread in frequencies of the stream a clear gap will form and will not be significantly filled in by upstream particles. This is essentially a condition on the velocity kicks being greater than the velocity dispersion of the stream. In this picture, the unperturbed stream density also plays an important role as if there are many particles upstream from the gap then the density contrast will be rapidly washed out. The stripping rate in the simulation we are examining decreases with time (see Section~\ref{Sect::FULL}) so the filling-in effect is less severe in our example than it potentially could be. The models of \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015} }%DIFAUXCMD
and \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015b} }%DIFAUXCMD
allow for a non-uniform unperturbed density distribution but with no stream velocity dispersion the stars in their model cannot pass through the stream to affect a gap.
}\DIFaddend 
...
\begin{equation}
M\propto r_s^{2.5}\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend \end{equation}
\DIFdelbegin \DIFdel{which roughly matches the scaling between mass and scale radius in }\DIFdelend \DIFaddbegin \DIFadd{This relation has been obtained from the publicly-available halo catalogues of the }\DIFaddend Via Lactea II \DIFaddbegin \DIFadd{simulations }\DIFaddend \citep{Diemand2008}. Keeping all other properties of the subhalo fly-by the same we simulate the effects of a $10^7M_\odot$ and $10^{7.5}M_\odot$ subhalo. In Figure~\ref{varying_mass} we show the gap profile \DIFdelbegin \DIFdel{after $t=880\Myr$}\DIFdelend \DIFaddbegin \DIFadd{$880\Myr$ after impact}\DIFaddend , the gap size as a function of time and the minimum and maximum density contrast as a function of time. \DIFdelbegin \DIFdel{As expected the }\DIFdelend \DIFaddbegin \DIFadd{Again we have used the smoothing kernel mentioned in Section~\ref{Sect::StreamGrowth} with an additional factor in the bandwidth depending on the subhalo mass $M$ of $0.5\log_{10}(M/10^6M_\odot)$. As expected from the results of \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015}}%DIFAUXCMD
, the }\DIFaddend gap size is deepest and grows fastest for the highest mass subhalo. In all cases the minimum density contrast plateaus in time and the value to which the density contrast plateaus decreases with increasing mass. The ratio between the asymptotic values of the density contrast for the $10^8M_\odot$ and the $10^7M_\odot$ case is observed to be $\sim4$.

\cite{ErkalBelokurov2015} showed that, for stream particles on a circular orbit, the central density contrast falls to zero like $1/t$ with the constant of proportionality scaling as $M/r_s^2$. \DIFdelbegin \DIFdel{However, here }\DIFdelend \DIFaddbegin \DIFadd{For all subhalo masses }\DIFaddend we observe that the density contrast plateaus in time \DIFdelbegin \DIFdel{. The discrepancy between our simulation andthe picture of \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015} }%DIFAUXCMD
seems to be due to two crucial differences. First, the stream has a non-zero velocity dispersion and each stripping event produces stream particles with a range of energies. This means very energetic upstream particles can pass through the stream }\DIFdelend and\DIFdelbegin \DIFdel{fill in a downstream gap. Inspection of Fig.~\ref{angfreq_dist} shows that if the parallel frequency kicks are greater than the spread in frequencies of the stream a clear gap will form and will not be significantly filled in by upstream particles. This is essentially a condition on the velocity kicks being greater than the velocity dispersion of the stream. Second, the unperturbed stream density plays an important role. If there are many particles upstream from the gap then the density contrast will be rapidly washed out. The stripping rate in the simulation we are examining decreases with time (see Section~\ref{Sect::FULL}) so the filling-in effect is less severe in our example than it potentially could be}\DIFdelend \DIFaddbegin \DIFadd{, as discussed in the previous section, is due to a combination of the stream density and the stream energy distribution. The time at which the density contrast reaches its plateau value decreases with decreasing mass}\DIFaddend .

\begin{figure*}
$$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{{{plots/fig13_different_masses}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{{{plots/fig14_different_masses}}}\DIFaddendFL $$
\caption{
Properties of the stream as a function of subhalo mass. The thick brown line corresponds to $10^8M_\odot$, the medium red line $10^{7.5}M_\odot$ and the thin orange line $10^7M_\odot$. The left panel shows the difference between the perturbed and unperturbed simulations in parallel angle \DIFaddbeginFL \DIFaddFL{$880\Myr$ }\DIFaddendFL after \DIFdelbeginFL \DIFdelFL{$t=880\Myr$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{impact}\DIFaddendFL , the middle panel shows the gap size as a function of time and the right panel shows the minimum and maximum density contrast as a function of time.
}
\label{varying_mass}
\end{figure*}
...
\begin{figure}
$$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{{{plots/fig14_different_strike_points}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{{{plots/fig15_different_strike_points}}}\DIFaddendFL $$
\caption{
Properties of the stream as a function of the location of the impact point. \textbf{Top row}: The top left panel shows the stream in the $(x,y)$ plane along with three dots showing the different impact points \DIFaddbeginFL \DIFaddFL{at the impact time}\DIFaddendFL . The top right panel shows the location of these strikes in parallel angle as well as the distribution of the stream in parallel frequencies and angles. \textbf{Middle row}: The middle two panels show the relative density contrast $\mathrm{d}N/\mathrm{d}\theta_{||} = \rho_\mathrm{perturbed}/\rho_\mathrm{unperturbed}-1$ in parallel angle space for two separate times \DIFaddbeginFL \DIFaddFL{($880\Myr$ and $5\Gyr$ after impact)}\DIFaddendFL . \textbf{Bottom row}: The bottom two panels show the gap size in parallel angle and the minimum and maximum density contrast as a function of time with the dashed lines showing the divergent peak at low unperturbed density.
}
...
\end{split}
\end{equation}
(see equation \eqref{linearevol} \DIFaddbegin \DIFadd{which defines $\Delta\btheta$}\DIFaddend ). The \DIFaddbegin \DIFadd{stripping time $t_s$ is the time since the particle was stripped from the progenitor. The }\DIFaddend specific model investigated by \cite{Bovy2014} and \cite{Sanders2014} simplified this to
...
We set the velocity-dispersion parameter \DIFaddbegin \DIFadd{($\sigma_v$) }\DIFaddend of the model by scaling that obtained in \cite{Bovy2014} for a similar, but lower mass, stream. As shown in \cite{SandersBinney2013a} for instance the properties of the stream in angles and frequencies scales with the mass of the progenitor, $M$, as $M^{1/3}$.

\DIFaddbegin \begin{table}
\caption{\DIFaddFL{Generative model parameters: we list each parameter in the modified perturbed generative model from Section~\ref{Sect::FULL}. The top section lists the parameters for the simple unperturbed stream model. The middle section lists the parameters added for the modified unperturbed model of Section~\ref{Sect::modelmods}. The bottom section lists the parameters used to model the perturbation. Note the parameters of the potential have not been listed.}}
\begin{tabular}{ll}
\DIFaddFL{$(\bs{x},\bs{v})$}&\DIFaddFL{Progenitor position \& velocity now}\\
\DIFaddFL{$\sigma_v$}&\DIFaddFL{Progenitor velocity-dispersion parameter}\\
\DIFaddFL{$t_d$}&\DIFaddFL{Disruption time}\\
\hline
\DIFaddFL{$w$}&\DIFaddFL{Evaporation-to-pericentric stripping weight}\\
\DIFaddFL{$\mu_\mathrm{peri}$}&\DIFaddFL{Centre of material stripped at pericentre}\\
\DIFaddFL{$\sigma_\mathrm{peri}$}&\DIFaddFL{Dispersion of material stripped at pericentre}\\
\DIFaddFL{$\mu_\mathrm{evap}$}&\DIFaddFL{Centre of evaporated material}\\
\DIFaddFL{$\sigma_\mathrm{evap}$}&\DIFaddFL{Dispersion of evaporated material}\\
\hline
\DIFaddFL{$M$}& \DIFaddFL{Subhalo mass}\\
\DIFaddFL{$r_s$}& \DIFaddFL{Subhalo scale radius}\\
\DIFaddFL{$\bs{w}$}& \DIFaddFL{Subhalo velocity at impact}\\
\DIFaddFL{$b$}& \DIFaddFL{Impact parameter}\\
\DIFaddFL{$t_g$}& \DIFaddFL{Impact time}\\
\end{tabular}
\label{Parameters}
\end{table}
...
Additionally, inspecting the distribution of $\Delta\theta_{||}/\Delta\Omega_{||}\approx t_s$ showed that the stripping \DIFdelbegin \DIFdel{time }\DIFdelend \DIFaddbegin \DIFadd{rate }\DIFaddend (averaged over a time longer than an orbital period of the progenitor) is not uniform\DIFdelbegin \DIFdel{in time}\DIFdelend , but instead decreases slowly over time \DIFaddbegin \DIFadd{(with decreasing stripping time)}\DIFaddend , such that a particle is more likely to have been stripped long ago. We modelled this by making $p(t_s)\propto t_s$ for both the periodic stripping and for the more steady evaporation. \DIFaddbegin \DIFadd{We found that this modification was necessary to even approximately reproduce the density of the stream near the gap. }\DIFaddend Compared to the simple model above, the distribution $p(\Delta\Omega_{||},t_s)$ cannot be separated as $p(\Delta\Omega_{||})\,p(t_s)$ in this model and the distribution of parallel frequency separation and stripping time looks like
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
where $\mathcal{N}(X|\mu,\sigma)$ is a Gaussian distribution with mean $\mu$ and width $\sigma$. The parameter $w=0.28$ is the weight, $T_R$ is the radial period, $t_R$ is the time since last pericentric passage and $\delta(t_s-nT_R-t_R)$ is a delta function expressing stripping at each pericentre passage. Fig.~\ref{UnpertNEW} shows the improvements made to the model such that it matches the simulation well. \DIFaddbegin \DIFadd{The full set of parameters used in the model are listed in Table~\ref{Parameters}.
}\DIFaddend 

\begin{figure}
$$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{{{plots/fig15_new_unperturbed_model}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{{{plots/fig16_new_unperturbed_model}}}\DIFaddendFL $$\\
\caption{Unperturbed stream model properties: the left panel shows the parallel angle separations against parallel frequency separations for particles from the simulation (in black) and samples of the generative model (in red). The solid line shows the mean parallel frequency separation for bins in parallel angle separation. The top right panel shows the parallel frequency separation distributions and the bottom right panel shows the stripping time distributions (where the colour-coding is the same as for the left panel). \DIFaddbeginFL \DIFaddFL{Larger stripping times mean the particles were stripped longer ago.}\DIFaddendFL }
\label{UnpertNEW}
\end{figure}
...
We generate 100,000 mock stream particles from the \DIFaddbegin \DIFadd{modified }\DIFaddend generative model
for the $M=10^8\,M_\odot$ impact. The density along the stream in the
simulation and of the mock stream is displayed in
Figure~\ref{config_density}. The grey, filled histogram shows the
...
very closely follows the simulated stream. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{All quantities are plotted as a function of the unperturbed $x$ position. This exaggerates the size of the perturbations. The perturbation as a function of the perturbed $x$ position is smaller and in this space the perturbed stream more closely follows the unperturbed stream track. The }\DIFaddend bottom of each panel
...
\begin{figure}
$$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{{{plots/fig16_config_density}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{{{plots/fig17_config_density}}}\DIFaddendFL $$
\caption{Density in a mock stream generated from the generative model described in the text (red) compared to that in the simulation (black) near the gap. The density of the unperturbed simulation is displayed in grey. The density in the simulation is well matched in the generative model.}
\label{config_density}
\end{figure}

\begin{figure*}
$$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\textwidth]{{{plots/fig17a_config_position}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\textwidth]{{{plots/fig18a_config_position}}}\DIFaddendFL $$\\
$$\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\textwidth]{{{plots/fig17b_config_velocity}}}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\textwidth]{{{plots/fig18b_config_velocity}}}\DIFaddendFL $$
...
We computed how the gap forms in the parallel angle (the angle along the stream) and showed that the distribution in this space can be simply related to the real-space properties of the gap. The evolution of the gap in angle-frequency space can be understood in the framework presented by \cite{ErkalBelokurov2015} and we verify that almost all the intuition developed in the circular orbit approach translates to results observed in the more realistic \DIFdelbegin \DIFdel{case.
}\DIFdelend \DIFaddbegin \DIFadd{eccentric orbit case. At times greater than approximately $10$ orbital periods of the progenitor orbit ($\sim6\Gyr$) after the impact, we find several exceptions to the \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015} }%DIFAUXCMD
picture: the density of the gap plateaus in time as stream material fills in the growing gap and the stream gap growth rate depends on the position along the stream at which the subhalo impact occurs, with gaps furthest from the progenitor growing fastest. For times less than $10$ orbital periods of the progenitor orbit our results corroborate the \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015} }%DIFAUXCMD
picture.
}\DIFaddend 

We showed how the structure of the gap varies as a function of the mass of the subhalo and the location along the stream the impact occurs. We found that, as expected \DIFaddbegin \DIFadd{from the work of \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015}}%DIFAUXCMD
}\DIFaddend , the parallel angle gap grows more slowly in time for lower mass subhaloes. The density contrast of the gap minimum plateaus to a constant value after a few $\Gyr$ for all inspected subhalo masses but the depth of this plateau increases with mass. The more interesting result is that the growth rate of the gap depends on the location along the stream where the impact occurs. We found that near the progenitor where the particles are mixed in energy the gap grows more slowly than far from the progenitor where the particles are well ordered in energy and so the gap forms on an already growing stream.
...
The framework presented here is appropriate for when an interaction between the stream and a subhalo is short compared to the orbital time of the stream such that the impulse approximation is appropriate. Instead of a full perturbative analysis we were able to simply consider instantaneous changes to the stream properties. However, it is potentially a small starting step to developing a scheme to handle more general perturbations to streams. The assumption of a smooth analytic galaxy model is probably inappropriate, particularly over the timescales on which streams evolve. \cite{Bonaca2014} presented a study of analysing streams from the Via Lactea simulation under assumptions on the smoothness \DIFaddbegin \DIFadd{and evolution }\DIFaddend of the halo\DIFdelbegin \DIFdel{and estimated that }\DIFdelend \DIFaddbegin \DIFadd{. They found that very massive subhalo encounters gave rise to minor overestimates in the halo mass whilst the time evolution of the potential produced }\DIFaddend biases of order \DIFdelbegin \DIFdel{$50\percent$ in the Milky Way mass could arise by falsely modelling the dark halo with a smooth analytic potential}\DIFdelend \DIFaddbegin \DIFadd{$20\percent$ in the halo mass}\DIFaddend . Additionally, recent results suggest that the mass of the Large Magellanic Cloud is much larger than previously believed \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Kallivayalil2013, Penarrubia2015}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Kallivayalil2013, Penarrubia2016} }%DIFAUXCMD
which suggests that its potential must be included as a perturbation in the modelling of the Galaxy}\DIFaddend . However, a counter to this is that many of the observed streams are very simple cold structures suggesting that the approximation of a smooth halo for the Milky Way is valid. Nonetheless it \DIFdelbegin \DIFdel{may be inevitable that}\DIFdelend \DIFaddbegin \DIFadd{is inevitable that, as data quality increase, }\DIFaddend one has to include perturbations on top of a smooth model to fit the known streams in the Galaxy and the angle-action framework is ideal for such an endeavour. \DIFaddbegin \DIFadd{We have demonstrated how a very simple perturbation can be included on top of a smooth angle-action model. }\DIFaddend These variables were originally introduced to handle perturbative solutions to the equations of the motion in the Solar system so seem highly appropriate for handling perturbations on Galactic scales.
...
Council of Canada for financial support of this project. DE acknowledges the \DIFdelbegin \DIFdel{finical }\DIFdelend \DIFaddbegin \DIFadd{financial }\DIFaddend support from the ERC. The research leading to these
...
Grant Agreement no. 308024. We thank Raymond Carlberg and Paul McMillan for useful comments on a draft. \DIFaddbegin \DIFadd{Additionally, we thank the anonymous referee for a very thorough report that improved the presentation of some of the results.
}\DIFaddend 
...
\section{Analytic approximation for action \DIFdelbegin \DIFdel{and angle }\DIFdelend kicks}\label{Appendix::Analytic}
...
\label{dJ_an}
\end{equation}
where $\delta L = \bs{L}\cdot\delta\bs{L}^g/|\bs{L}|$ the change in the angular momentum. The change in $J_R$ comes from the spherical approximation and the change in $J_z$ from the assumption that it is a harmonic oscillator in the $z$ direction. We show these analytic approximations in Fig.~\ref{aa_nz} along with the kick distributions overlaid on \DIFdelbegin \DIFdel{the difference plots of Fig. ~\ref{tilted_diff_map}. The }\DIFdelend \DIFaddbegin \DIFadd{difference histograms between the perturbed and unperturbed stream. As with the angle and frequency distributions in Figure~\ref{tilted_diff_map} we observe an under-density at the centre of the action distributions. The exact geometry of the under and over-densities is complex and clearly depends on the nature of the stream track and subhalo properties. Inspection of equation~\ref{dJ_an} shows that the direction of the action kicks depends on the arbitrary direction of the velocity kicks and the properties of the stream at impact. The }\DIFaddend analytic approximations recover the magnitude of the kicks within a factor of $\sim1.5$ but are not nearly as impressive as the frequency kick analytic expressions. These approximations work better for streams confined to the meridional plane. However, we see from the lower panels of Fig.~\ref{aa_nz} that the correlations between the action kicks are well recovered and explain the observed structure in the action difference plots.
...
%DIFAUXCMD
\DIFdel{where $E$ is the energy of the oscillation. We see that near a turning point ($v_i\approx0$) the angle kicks are largest and of order $\delta v_i/\sqrt{E}$ and near the midpoints ($x_i\approx0$) the angle kicks are approximately zero. As expected the magnitude of the angle kicks are phase-dependent. Using equation~}%DIFDELCMD < \eqref{dOm_an} %%%
\DIFdel{we see that
}\begin{displaymath}
...
\caption{
Analytic action kick approximation. In the \textbf{top panels} the red shows the approximate analytic expressions from equation~\protect\eqref{dJ_an} and the black is computed from the \DIFdelbeginFL \DIFdelFL{simulations}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{simulation}\DIFaddendFL . In the \textbf{bottom panels} we overlay the action kick distributions on the difference \DIFdelbeginFL \DIFdelFL{plots from Fig}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{histograms between the perturbed and unperturbed streams}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{~}%DIFDELCMD < \protect%%%
\DIFdelFL{\ref{tilted_diff_map}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The bin spacing for the difference histograms is $0.7\kpc\kms$}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{Clearly }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{As with }\DIFaddendFL the \DIFaddbeginFL \DIFaddFL{angle and frequency difference distributions there is a clear gap formed at the impact centre. The }\DIFaddendFL analytic approximation explains the correlations observed in the simulation.
}
\label{aa_nz}
\end{figure*}
