...
%DIF ADD ../stream_gaps_draft.tex                                    Fri Dec 11 13:34:27 2015
\usepackage{times}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsbsy}
...
\citep{Giocoli2010,Gao2011}}%DIFAUXCMD
...
\begin{figure}
$$\DIFaddFL{\includegraphics[width=\columnwidth]{{{plots/fig0_geometry}}}}$$
...
. Therefore }\DIFaddend it is this quantity which has the largest effect on the future structure of the stream.
...
\begin{figure*}
$$\DIFaddFL{\includegraphics[width=\textwidth]{{{plots/fig2_nfw_plummer_comparison}}}}$$
\caption{
...
$$\DIFaddFL{\includegraphics[width=\columnwidth]{{{plots/fig3_tilted_gap_morphology}}}}$$
\caption{
...
\begin{figure*}
$$\DIFaddFL{\includegraphics[width=\textwidth]{{{plots/fig4_tilted_diff_map}}}}$$
\caption{
...
\begin{figure*}
\DIFaddbeginFL $$\DIFaddFL{\includegraphics[width=\textwidth]{{{plots/fig6_tilted_angfreq_kicks}}}}$$
\caption{
\DIFaddFL{Angle and frequency kicks: the coloured points are a random sample of $1000$ particles from the stream and show the kicks found from the simulations whilst the black lines show those calculated under the impulse approximation. The functional form for the kicks in both the angles and frequencies are very similar to that in velocities. Also, the angle and frequency kicks computed from the simulation match the numerical results well.
}}
\label{kicks_nzero}
\end{figure*}

\begin{figure*}
\DIFaddendFL $$\includegraphics[width=\textwidth]{{{plots/fig7_tilted_model_perturbed}}}$$
\caption{
Frequency distributions for the perturbed and unperturbed stream model. The red histograms show the unperturbed stream, the green show the perturbed stream and the blue show the unperturbed stream kicked in frequencies using equations~\protect\eqref{eq:kickfreqangle}. \DIFaddbeginFL \DIFaddFL{The left three panels show the distributions in the orbital frequencies with respect to the gap centre. The fourth panel shows the frequency distribution along the stream direction $\hat{\boldsymbol{n}}$ whilst the fifth panel shows the distribution perpendicular to this.
}\DIFaddendFL }
\label{model_nzero}
\end{figure*}
...
\DIFaddendFL $$\includegraphics[width=\columnwidth]{{{plots/fig8_ang_later_times}}}$$
\caption{
...
\DIFaddbegin \begin{figure}
$$\DIFaddFL{\includegraphics[width=\columnwidth]{{{plots/fig12_real_vs_angle_properties}}}}$$
\caption{
...
We computed how the gap forms in the parallel angle (the angle along the stream) and showed that the distribution in this space can be simply related to the real-space properties of the gap. The evolution of the gap in angle-frequency space can be understood in the framework presented by \cite{ErkalBelokurov2015} and we verify that almost all the intuition developed in the circular orbit approach translates to results observed in the more realistic case. \DIFaddbegin \DIFadd{At times greater than approximately $10$ orbital periods of the progenitor orbit ($\sim6\Gyr$) after the impact, we find several exceptions to the \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015} }%DIFAUXCMD
picture: the density of the gap plateaus in time as stream material fills in the formed gap and the stream gap growth rate depends on the position along the stream the subhalo impact occurs with gaps furthest from the progenitor growing fastest. For times less than $10$ orbital periods of the progenitor orbit our results corroborate the \mbox{%DIFAUXCMD
\cite{ErkalBelokurov2015} }%DIFAUXCMD
picture.
}\DIFaddend 
