...
\caption{
Angle and frequency distributions for the perturbed and unperturbed stream model: the \textbf{top row} shows the angle distributions and the \textbf{bottom row} the frequency distributions. The red histograms show the unperturbed stream $t=0.88\Gyr$ after impact, the green show the perturbed stream and the blue show the unperturbed stream kicked in angle and frequency space and evolved to the current time.
}
\label{model_nzero}
\end{figure*}
...
\begin{figure*}
$$\includegraphics[width=\textwidth]{{{plots/fig10_tilted_analytic_action}}}$$
\caption{
...
\cite{ErkalBelokurov2015} showed that, for stream particles on a circular orbit, the central density contrast falls to zero like $1/t$ with the constant of proportionality scaling as $M/r_s^2$. However, here we observe that the density contrast plateaus in time. Inspired by the results of \cite{ErkalBelokurov2015} it appears the minimum density contrast plateau value for our particular simulation goes like $\sim (1+A M)^{-1}$, where $A$ is a constant. However, we are unable to validate this result, but here we will describe expected qualitative behaviour.

The discrepancy between our simulation and the picture of \cite{ErkalBelokurov2015} seems to be due to two crucial differences. First, the stream has a non-zero velocity dispersion and each stripping event produces stream particles with a range of energies. This means very energetic upstream particles can pass through the stream and fill in a downstream gap. However, provided the energy distribution of released stream particles does not change significantly in time, there will always be some perturbed particles that are equally energetic to the most energetic upstream particles. This means the perturbed particles will not be reached by the unperturbed particles and the gap will skew in time. Second, the unperturbed stream density plays an important role. If there are many particles upstream from the gap then the density contrast will be rapidly washed out. The stripping rate in the simulation we are examining decreases with time (see Section~\ref{Sect::FULL}) so the filling-in effect is less severe.

We performed several experiment with approximate analytic models and numerical models to investigate the impact of the above two effects. All models appear to exhibit a plateau in the central gap density contrast as $t\rightarrow\infty$ but for some models (low mass subhalo or high stream velocity dispersion, $\sigma_v$) the density contrast exhibits a minimum before plateauing at a higher density contrast. It seems that the exact behaviour is controlled by the ratio $|\delta \boldsymbol{v}^g|/\sigma_v$. Although the central gap density exhibits this behaviour, due to the above described effects the gap skews in time such that the central density contrast does not correspond to the minimum density contrast. In the inspected models this minimum density contrast does not exhibit a minimum but instead steadily decays to a plateau value that does not seem to depend upon the stripping rate distribution very sensitively.
...
\end{split}
\end{equation}
where $\mathcal{N}(X|\mu,\sigma)$ is a Gaussian distribution with mean $\mu$ and width $\sigma$. $w=0.28$ is the weight, $T_R$ is the radial period, $t_R$ is the time since last pericentric passage and $\delta(t_s-nT_R-t_R)$ is a delta function expressing stripping at each pericenter passage. Fig.~\ref{UnpertNEW} shows the improvements made to the model such that it matches the simulation well.
...
We closed by presenting a fully generative model of a perturbed stream in galactocentric Cartesian coordinates based on the model presented in \cite{Bovy2014} and \cite{Sanders2014}, and shown that the model matches the simulation qualitatively very well. We found it necessary to modify the simple model by modifying making the stripping rate increase in time, and including two populations of stripped particles: one population are stripped around pericentre with large energy shifts from the progenitor whilst the other population evaporate form the cluster continuously with smaller energy shifts.
...
Council. We also thank N. Wyn Evans for calculating the velocity perturbations due to a Hernquist profile in the impulse approximation.
