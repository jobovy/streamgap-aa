...
M\propto r_s^{2.5},
\end{equation}
which roughly matches the scaling between mass and scale radius in Via Lactea II \citep{Diemand2008}. Keeping all other properties of the subhalo fly-by the same we simulate the effects of a $10^7M_\odot$ and $10^{7.5}M_\odot$ subhalo. In Figure~\ref{varying_mass} we show the gap profile after $t=880\Myr$, the gap size as a function of time and the minimum and maximum density contrast as a function of time. As expected the gap size is deepest and grows fastest for the highest mass subhalo. In all cases the minimum density contrast plateaus in time and the value that the density contrast plateaus to decreases with mass. As shown in \cite{ErkalBelokurov2015} for a direct impact the spatial gradient of the velocity kicks around the impact point scales as $(M/r_s)^2$. From the approximate relation of equation~\eqref{dOm_an} we find that the gradient of the frequency kicks with the parallel angle at the gap centre also scales with this quantity. Therefore, it naturally follows from equation~\eqref{densitycontrast} that for a stream with uniform parallel-angle density and constant frequencies $\rho_\mathrm{min}/\rho_0\sim(M/r_s)^{-1}\sim M^{-0.2}$ at large times. Therefore, we anticipate that there is a factor $\sim1.6$ difference in depth between the $10^8M_\odot$ and $10^7M_\odot$ cases.  However, equation~\eqref{densitycontrast} also predicts that this density contrast will fall to zero like $1/t$ whilst we observe the density contrast plateauing in time. The ratio between the asymptotic values of the density contrast for the $10^8M_\odot$ and the $10^7M_\odot$ case is observed to be $\sim4$ in Figure~\ref{varying_mass}. We cannot explicitly justify why the ratio does not scale according to our prediction or why the density contrast plateaus, but it is probably due to the velocity dispersion and density profile of the unperturbed stream.
...
\begin{figure}
$$\includegraphics[width=\columnwidth]{new_unperturbed_model.png}$$\\
