...
We found that the simple model described above did not completely describe the simulation such that several modifications seemed necessary. Firstly, the stream distribution in $\Omega_{||}$ is not well described by a Gaussian, but instead seemed better fitted by two Gaussians. Inspection of the stream distribution in $\Delta\Omega_{||}$ and $\Delta\theta_{||}$ showed the stream consists of material stripped in two separate ways: $\sim72\percent$ of the material is stripped around pericentric passage and this material forms a broad peak in $\Delta\Omega_{||}$ that is separated from the progenitor by $\mu_\mathrm{peri}=0.31\Gyr^{-1}$ with a width of $\sigma_\mathrm{peri}=0.05\Gyr^{-1}$. The other $\sim28\percent$ material that is more continuously stripped, or evaporates, from the progenitor. This material forms a narrower peak in $\Delta\Omega_{||}$ that is less separated from the progenitor at $\mu_\mathrm{evap}=0.25\Gyr^{-1}$ with a width of $\sigma_\mathrm{evap}=0.023\Gyr^{-1}$.
...
We calculate the kicks in the angles and frequencies in the same way as before, but instead of using a spline for the unperturbed stream track, we use the stream track computed from the model.

COULD WE HAVE PLOTS HERE OF THE STRIPPING RATE? THE FREQUENCY DISTRIBUTION?

In Fig.~\ref{FullModel} we show the full perturbed and unperturbed stream model density alongside the simulation, along with the differences between the perturbed and unperturbed Galactocentric Cartesian coordinates for the model and the simulation. We find a very good match between the unperturbed stream densities even for our still over-simplified stripping model. The differences in the unperturbed and perturbed coordinates match well. We have seen that the kicks are applied correctly in angle-frequency space so this result demonstrates our ability to map these kicks correctly into observable space. Additionally, the shape of the gap is well recovered by the model.
